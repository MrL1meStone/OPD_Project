...
class MainWindow(Tk):
    ...
    def add_text(self):
        ...
        def check():
            ...
            right = check_output(input_func=question['args'], output_func=question['func'])
            if right:
                print(right)
        ...
        ttk.Button(text='< Меню', command=self.menu).pack(pady=(100, 0), padx=(100, 0), anchor='w')
        ...
        errors = StylishText(error_frame,height=20, width=30,fg=self.style.error,style=self.style)
        errors.pack()
        ttk.Button(error_frame,text='Сдать').pack(pady=10)


        code_frame = ttk.Frame(code_space)
        numbers = StylishText(code_frame, height=20, width=3, style=self.style, fg = 'gray',selection=False)
    ...
    def styles(self):
        ...
        StylishText(widgets_frame, text=f'\n{'Пример текста': ^20}\n',
                    ...
                    disabled=True).pack(side='top', fill='x')
